  ...
  return <><style>{dashboardStyles}</style><div className="dashboard">
    ...
    <main id="top" className="content">
      ...
      <div className="dashboard-grid">
        ...
        <section className="air panel"><div className="title-row"><h2><span className="title-icon"><Icon name="wind" size={14} /></span> 대기질 정보</h2><span className="good">{airQuality.data?.overallGrade ?? (airQuality.loading ? '조회 중' : 'AirKorea')}</span></div><p className="updated">{airQuality.data ? `AirKorea · ${airQuality.data.stationName} 측정소 · ${airQuality.data.measuredAt ?? '측정 시각 미제공'}` : airQuality.loading ? '가까운 측정소의 최신 자료를 불러오는 중…' : '내 위치를 확인하면 대기질이 표시됩니다.'}</p>{airQuality.error && <p className="api-message" role="alert">{airQuality.error}</p>}<div className="air-values">{[{ label: '미세먼지 PM10', metric: airQuality.data?.pm10, unit: '㎍/㎥' }, { label: '초미세먼지 PM2.5', metric: airQuality.data?.pm25, unit: '㎍/㎥' }, { label: '오존 (O₃)', metric: airQuality.data?.ozone, unit: 'ppm' }].map(({ label, metric: airMetric, unit }) => <div key={label}><Icon name="activity" /><small>{label}</small><strong><AnimatedNumber text={airMetric?.value == null ? '—' : `${airMetric.value} ${unit}`} /></strong><span>{airMetric?.grade ?? '정보 없음'}</span></div>)}</div></section>
